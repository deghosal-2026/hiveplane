...
- [x] [#199](https://github.com/deghosal-2026/hiveplane/issues/199) — M30-01 — Task router: cheap-model classifier maps a task to a candidate workload/pipeline from the certified catalog
- [x] [#200](https://github.com/deghosal-2026/hiveplane/issues/200) — M30-02 — Router guardrails: only certified workloads are eligible; confidence threshold + explicit fallback/refusal
- [x] [#201](https://github.com/deghosal-2026/hiveplane/issues/201) — M30-03 — Router explanations: why this workload was chosen (candidate scores recorded in the run story)
- [x] [#202](https://github.com/deghosal-2026/hiveplane/issues/202) — M30-04 — Agent-as-tool: expose a certified workload as a callable tool to other workloads
- [x] [#203](https://github.com/deghosal-2026/hiveplane/issues/203) — M30-05 — Propagation: nested agent-as-tool calls inherit budget, policy, and certification checks; results are attributed
- [x] [#204](https://github.com/deghosal-2026/hiveplane/issues/204) — M30-06 — Recursion/depth limits and cycle detection for agent-as-tool calls
- [x] [#205](https://github.com/deghosal-2026/hiveplane/issues/205) — M30-07 — A2A interop (stretch): Agent2Agent protocol adapter for cross-plane calls behind a feature flag
- [x] [#206](https://github.com/deghosal-2026/hiveplane/issues/206) — M30-08 — Tests: routing accuracy on a labeled set, refusal on low confidence, nested-call propagation, depth limits

**Test ticket:** [x] [#207](https://github.com/deghosal-2026/hiveplane/issues/207) — Test cases for Smart Task Router & Agent-as-Tool
...
**Acceptance criteria:**
- [x] A plain-language task routes to the expected workload on a labeled test set above an agreed threshold
- [x] An uncertified workload is never a routing candidate
- [x] Low-confidence tasks are refused with a reason, not guessed
- [x] An agent-as-tool call carries budget/policy/certification context and is blocked when the nested workload is uncertified
- [x] Recursive agent-as-tool calls beyond the depth limit are rejected
...
> **Status:** M30 complete. `hiveplane.router` (catalog, classifier, engine,
> decision store), `hiveplane.agent_tools` (registry, invoker, invocation store),
> the A2A stretch adapter behind `HIVEPLANE_A2A__ENABLED`, migration `0009`
> (`router_decisions`, `agent_tool_invocations`), `Run.agent_tool_origin`, the
> route/agent-tool API, and the `hiveplane route` / `hiveplane agents` CLI
> landed. All tests pass with a database; coverage 95% total, ruff and mypy
> strict clean. Issues #199–#207 closed; changes committed and pushed.
...
- [x] All M29–M30 issues done and closed
