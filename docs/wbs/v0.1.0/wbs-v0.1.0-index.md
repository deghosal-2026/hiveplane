...
> **Progress:** Parts 1-11 complete and M22 complete (M1-M22, #2-#55, #82-#91) — 87/112 issues closed. M23 re-planned into phases P0-P4 (P2 split into ordered sub-phases P2a-P2d). **P0 (Design & Planning), P1 (LLM & Agent Enablement), P2a (Wiring fixes — the audit chain, #134-#139/#141/#142/#144), P2b (Corpus & provider reconciliation, #98/#123/#140), P2c (Real certification, #131/#129/#109), the whole of P2d (subprocess sandbox #110, durable resume #111, durable LangGraph checkpoint #122, trace story #124, honest `/readyz` #130) and P3's #143 (fixtures copied into the image) are done — all three example workloads certify at production threshold through the real adapter path, sandboxed runs execute in a capped child, interrupted runs recover on boot, and `/readyz` is honest.** **8 open issues remain** across P4 (field test: #99, #121, #56, #57, #58, #100, #101, #102), plus M24. P3 (Docker Test Track) is complete — `make docker-test` run `20260924T235452Z` passed 25/25 against the real local model, screenshots are embedded in the user guide, and the report carries an Issues/Learnings section. (#59 closed as not planned — the nightly-simulator/regression-alert scope is not pursued for v0.1.0.) M23-M24 remain.
