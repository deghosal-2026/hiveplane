...
- [ ] #93 — Docker test cases, dummy data, and setup (layered `tests/docker/`, `deploy/testdata/`, OMLX-on-host local profile + fake-webhook compose profile) — needs #143. **Zero skips:** the suite requires a reachable local LLM and must fail without it; `scripts/docker-test.sh` preflights the endpoint. L0 (`test_container_fixtures.py`) and L7 (`test_llm_matrix.py`, `conftest.py`) scaffolded.
