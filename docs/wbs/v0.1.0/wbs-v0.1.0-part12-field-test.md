...
- [ ] #93 — Docker test cases, dummy data, and setup (layered `tests/docker/`, `deploy/testdata/`, OMLX-on-host local profile + fake-webhook compose profile) — needs #143. **Zero skips:** the suite requires a reachable local LLM and must fail without it; `scripts/docker-test.sh` preflights the endpoint. **Done:** L0 (`test_container_fixtures.py`), L1 (`test_stack_health.py`), L2 (`test_api_contract.py`), L4 (`test_control_loop.py`), L5 (`test_governance.py`), L6 (`test_durability.py`), L7 (`test_llm_matrix.py` + `conftest.py`); negative workloads (`uncertified-agent`, `model-swap-agent`, `regressed-agent` + `examples/regressed_agent.py`) and governance fixtures (`deploy/testdata/governance/{over-budget,destructive-call,large-output}.json`); run evidence under `field_test/v0.1.0/docker/` (committed) with `scripts/docker_report.py` rendering `docs/field-test/v0.1.0/DOCKER_TEST_REPORT.md`. **Remaining:** L3 UI. (L5 covers the policy/approval surface; end-to-end budget/shaping enforcement needs priced usage or the fake profile — tracked with #94.)
- [ ] #94 — Docker test execution (one-command runner, structured results, CI exit codes) — needs #93 + #143. `make docker-test` / `scripts/docker-test.sh` now bring the stack up, wait for `/readyz`, seed tools, run `pytest tests/docker -m docker`, and tear down; local-LLM preflight is fail-fast.
