"""Pipeline migration 0008 upgrade/downgrade tests (Postgres-gated)."""
...
def test_0008_adds_and_drops_pipeline_tables(pg_engine: Engine) -> None:
    config = _config()
...
def test_0008_is_idempotent(pg_engine: Engine) -> None:
    config = _config()
    command.upgrade(config, "head")
    command.upgrade(config, "head")
    assert set(inspect(pg_engine).get_table_names()) >= _TABLES
