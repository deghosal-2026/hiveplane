"""Migration 0009 (router + agent tools) upgrade/downgrade tests (Postgres-gated)."""
...
def test_0009_adds_and_drops_router_tables(pg_engine: Engine) -> None:
    config = _config()
...
def test_0009_is_idempotent(pg_engine: Engine) -> None:
    config = _config()
    command.upgrade(config, "head")
    command.upgrade(config, "head")
    assert set(inspect(pg_engine).get_table_names()) >= _TABLES
